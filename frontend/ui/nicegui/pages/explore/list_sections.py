    ...
    *,
    ...
    def _render_course_item(course: dict[str, Any], *, item_classes: str) -> None:
        with ui.element("div").classes(item_classes):
            ...
            tracked = deps.state.tracking_by_course_id.get(course_id)
            url = str(course.get("url") or "").strip()
            can_edit = bool(deps.is_admin or (str(course.get("created_by") or "") == deps.username))
            card_vm = map_course_card_view(
                course_row=course,
                tracked_row=tracked if isinstance(tracked, dict) else None,
                review_summary_row=deps.state.course_review_summary_by_course_id.get(course_id),
                recommendation_summary_row=deps.state.course_recommendation_summary_by_course_id.get(course_id),
            )
            render_course_card(
                course_row=course,
                tracked_row=tracked if isinstance(tracked, dict) else None,
                ...
                can_edit=can_edit,
                has_url=bool(url),
                actions=deps.course_actions_builder(course, course_id, url),
                is_tracked_course=lambda cid: int(cid) in deps.state.tracking_by_course_id,
                resolve_status_value=resolve_tracking_status_value,
                on_set_status=deps.on_set_tracking,
                on_clear_status=deps.on_clear_tracking,
    ...
    def _render_path_item(path: dict[str, Any], *, item_classes: str) -> None:
        with ui.element("div").classes(item_classes):
            ...
            is_tracked = path_id in deps.state.selected_by_path_id
            can_edit = bool(deps.is_admin or (str(path.get("created_by") or "") == deps.username))
            card_vm = map_path_card_view(
                path_row=path,
                is_tracked=is_tracked,
                detail=deps.state.selected_detail_by_path_id.get(path_id),
                tracking_by_course_id=dict(deps.state.tracking_by_course_id or {}),
                review_summary_row=deps.state.path_review_summary_by_id.get(path_id),
                recommendation_summary_row=deps.state.path_recommendation_summary_by_id.get(path_id),
            )
            track_toggle_label = "Unselect" if is_tracked else "Select"

            async def _on_track_toggle() -> None:
                await deps.on_toggle_path_selection(path_id)

            async def _open_path_details_inline() -> None:
                deps.open_path_details_dialog(path, card_vm)
    ...
    def _render_article_item(article: dict[str, Any], *, item_classes: str) -> None:
        with ui.element("div").classes(item_classes):
            ...
            vm = map_article_card_view(
                article_row=article,
                review_summary_row=deps.state.article_review_summary_by_article_id.get(article_id),
            )
            actions = build_article_card_actions(
                article_row=article,
                on_open_details=deps.open_article_details,
            )
            render_article_card(
                article_row=article,
                is_new=vm.is_new,
    ...
    if shown_courses:
        ...
        render_explore_course_spotlight(
            enabled=deps.feature_explore_cinema,
        ...
        render_courses_catalog(
            ...
            show_featured=not deps.feature_explore_cinema,
