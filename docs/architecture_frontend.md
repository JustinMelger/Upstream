...
The supported frontend is a Vite-built React application in `frontend/react`. It replaces the Python NiceGUI runtime. Earlier design documents remain visual references, not active framework instructions.
...
See [UX refinement record](frontend_ux_refinement.md) for delivery evidence.

## Visual discovery release
...
Article summaries now use an optional description field. Share/edit previews derive from current form values and update after draft restoration/discard; only a successful sessionStorage write displays the saved message. See [visual discovery implementation](visual_discovery_implementation.md) for verification and rollout.
