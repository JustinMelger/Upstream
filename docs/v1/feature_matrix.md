...
| Tracking | `/tracking`, `/tracking/stats`, `/tracking/stats/users` | learning/home/profile surfaces | `tracking`, `user_paths` | self-service for own tracking, admin for aggregate views | `keep` | v1 scope is courses plus selected path progress only |
| Teams | `/teams`, `/teams/mine`, `/teams/{id}`, member endpoints, `/teams/{id}/activity` | `/teams` | `teams`, `team_members` | member-only visibility; owner/admin member management | `keep` | treat as basic workspace, not full collaboration |
...
| Course/path recommendations | course/path recommendation APIs and summary endpoints | secondary card/detail/home actions only | `course_recommendations`, `path_recommendations` | authenticated; owner/admin moderation | `defer` | implemented, but cut from v1 because it overlaps confusingly with reviews |
...
| Team audience/scoped sharing | partial roadmap only | not canonical | not fully present | n/a | `defer` | do not describe as a v1 feature |
